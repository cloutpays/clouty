...
console.log('devy', dev);
const valid = !dev
  ? firebaseConfig && firebaseConfig.apiKey && firebaseConfig.projectId
  : firebaseDevConfig &&
    firebaseDevConfig.apiKey &&
    firebaseDevConfig.projectId;

try {
  if (!firebase.apps.length) {
    firebase.initializeApp(!dev ? firebaseConfig : firebaseDevConfig);
  }
} catch (e) {
  console.log(e);
}
  ...
  signup(info) {
    return firebaseAuth()
      ...
      .catch((error, user) => {
        ...
        console.log('error', user, errorCode, errorMessage);
      });
  }
  login(provider, info) {
    switch (provider) {
